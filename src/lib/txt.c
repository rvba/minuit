...
void txt_draw_3d(char *string,float pos[3])
{
	...
	glPushMatrix();
		...
		float ob_pos[3];   // txt pos
		float ob_axis[3];  // up axis
		float cam_pos[3];  // camera pos
		float look[3];
		float right[3];
		float up[3];
		...
		if(1)
		{
			...
			vcp(ob_pos,pos);
			// set cam_pos
			vcp(cam_pos,camera->eye);
			// sey ob_axis
			vset(ob_axis,0,0,1);//z

			vsub(look,cam_pos,ob_pos);
			vnormalize(look);

			vcp(up,ob_axis);
			//float *up = ctx->app->camera->up;

			vcross(right,up,look);
			vnormalize(right);

			vcross(look,right,up);
		}
		else
		{
			vcp(ob_pos,pos);
