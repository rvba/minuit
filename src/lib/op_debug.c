...
void op_debug_mouse(void)
{
	...
	glPushMatrix();
	glLoadIdentity();
		//glTranslatef(width-offset,height-offset,0);
		glTranslatef(width-offset,100,0);
		glScalef(s,s,s);

		glTranslatef(0,-vert,0);
		snprintf(txt,l,"camera is_moving %d",C->camera->is_moving);
		txt_draw_direct_2d(txt,pos,color,txt_scale);
		//skt_col_set(1,1,1);
		glTranslatef(0,-vert,0);
		txt_draw_direct_2d("mouse",pos,color,txt_scale);

		// is_moving
		glTranslatef(0,-vert,0);
		snprintf(txt,l,"is_moving %d",mouse->is_moving);
		txt_draw_direct_2d(txt,pos,color,txt_scale);

		//brick left
		glTranslatef(0,-vert,0);
		debug_mouse_brick(mouse,"left",txt);
		txt_draw_direct_2d(txt,pos,color,txt_scale);

		// x y
		glTranslatef(0,-vert,0);
		snprintf(txt,l,"x%d y%d",mouse->x,mouse->y);
		txt_draw_direct_2d(txt,pos,color,txt_scale);

		// dx dy
		glTranslatef(0,-vert,0);
		snprintf(txt,l,"dx%d dy%d",mouse->dx,mouse->dy);
		txt_draw_direct_2d(txt,pos,color,txt_scale);

		// delta_x delta_y
		glTranslatef(0,-vert,0);
		snprintf(txt,l,"delta x %d y %d",mouse->delta_x,mouse->delta_y);
		txt_draw_direct_2d(txt,pos,color,txt_scale);

		// sign
		glTranslatef(0,-vert,0);
		snprintf(txt,l,"sign x %d y %d",mouse->sign_x,mouse->sign_y);
		txt_draw_direct_2d(txt,pos,color,txt_scale);

		// last
		glTranslatef(0,-vert,0);
		snprintf(txt,l,"last %d %d",mouse->last_x,mouse->last_y);
		txt_draw_direct_2d(txt,pos,color,txt_scale);

		// delta_x delta_y
		glTranslatef(0,-vert,0);
		snprintf(txt,l,"delta time %f",mouse->clic_clock->delta);
		txt_draw_direct_2d(txt,pos,color,txt_scale);
